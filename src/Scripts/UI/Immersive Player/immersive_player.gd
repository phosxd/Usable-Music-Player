...
func _ready() -> void:
	...
	var topbar = SessionManager.main_scene.get_node('%Topbar') 
	var rightbar = SessionManager.main_scene.get_node('%Right Sidebar')
	if sidebar is Control:
		sidebar.hide()
	if topbar is Control:
		topbar.hide()
	if rightbar is Control:
		rightbar.hide()
